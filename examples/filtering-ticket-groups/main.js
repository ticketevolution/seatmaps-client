...
const createQuantity = (quantity) => {
  ...
  element.append(
    document.createTextNode(`${quantity} ticket${quantity > 1 ? "s" : ""}`),
  );
  return element;
};
...
const createTicketGroupListItem = (ticketGroup) => {
  ...
  ticketGroupListItem.appendChild(
    createHeading(ticketGroup.section, ticketGroup.row),
  );
...
const filter = () =>
  allTicketGroups
    ...
    .filter(
      (ticketGroup) =>
        typeFilter.size === 0 || typeFilter.has(ticketGroup.format),
    );
...
ticketGroupsElement.addEventListener("mouseover", (event) => {
  const element = event.path.find(
    (path) =>
      path.classList && path.classList.contains("ticket-groups-list-item"),
  );
  if (element) {
...
const fetchTicketGroups = () =>
  fetch("./node_modules/@-/mock-data/data/ticket-groups-1591449.json").then((response) =>
    response.json(),
  );
...
window.onload = () => {
  ...
  fetchTicketGroups().then((ticketGroups) => {
    allTicketGroups = ticketGroups.sort(
