...
import { State, Props, Manifest } from './types'
...
describe('TicketMap', () => {
  ...
  beforeEach(() => {
    ...
    mapResponse = [ '<svg><path data-section-id="foo bar" /></svg>', { status: 200 } ]
  ...
  it('fetches the map associated with the venue and config IDs', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('https://maps.ticketevolution.com/1/2/map.svg')
  })
  ...
  it('fetches the manifest associated with the venue and config IDs', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('https://maps.ticketevolution.com/1/2/manifest.json')
  })
