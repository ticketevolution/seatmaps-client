...
import { State, Props, Percentiles } from './types'
...
describe('$costRanges', () => {
  ...
  it('adds ticket groups to their associated cost range', () => {
    expect($costRanges(state, props)[0].ticketGroups).toContainEqual(ticketGroupFooBar)
    expect($costRanges(state, props)[1].ticketGroups).toContainEqual(ticketGroupBizBaz)
  })
})
