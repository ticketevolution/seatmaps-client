...
import SeatmapFactory, { extractConfigurationFromOptions, validateOptions } from './index'
...
describe('SeatmapFactory', () => {
  ...
  describe('build', () => {
    ...
    it('throws an error if rootElementId is not provided', () => {
      expect(() => factory.build('')).toThrow('Seatmaps must be initialized with a DOM element.')
    })

    it('throws an error if rootElementId is not associated with a DOM element', () => {
      expect(() => factory.build('foo')).toThrow('Seatmaps must be initialized with a DOM element.')
    })

    it('mounts the TicketMap component into the root element', () => {
      /* eslint-disable @typescript-eslint/no-explicit-any */
      (global as any).document.getElementById = jest.fn(() => (global as any).document.body)
  ...
  describe('validateOptions', () => {
    ...
    it('throws an error if any required properties are missing from options', () => {
      /* eslint-disable @typescript-eslint/no-object-literal-type-assertion */
      expect(() => validateOptions({ venueId: '4' } as Props)).toThrow(`Seatmap configuration requires a 'configurationId' value.`)
      /* eslint-enable @typescript-eslint/no-object-literal-type-assertion */
    })
  })
})
